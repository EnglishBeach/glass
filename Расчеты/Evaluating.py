...
def result(base, delta):
    """
    Massive must be with names
    :param base:  Need massive of temperatures. temp:row1:row2...
    :param delta: (List1) any diapason temperatures
    :return: List [[temperature] : [value]]
    ...
    for i2 in range(1, len(base)):
        ...
        for i in delta:
            ...
            if base[0][h] <= i <= base[0][h + 1]:
                ...
                '''
                dx = i - base[0][h]
                dfdx = (base[i2][h + 1] - base[i2][h]) / (base[0][h + 1] - base[0][h])
                f0 = base[i2][h]
                a = dfdx * dx + f0
                if i2 == 1:
                    out[0].append(i)
